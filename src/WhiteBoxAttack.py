...
class CarliniLInfMethod(EvasionAttack):
    ...
    def _loss_gradient(self, z_logits, target, x_adv, x_adv_tanh, clip_min, clip_max):  # lgtm [py/similar-function]
        ...
        for i in range(len(loss_gradient)):
            loss_gradient_new.append([])
            for j in range(32):
                loss_gradient_new[-1].append([])
                for k in range(64):
                    loss_gradient_new[-1][-1].append(0.0)
            for j in range(32):
                loss_gradient_new[-1].append([])
                for k in range(64):
                    # loss_gradient_new.append([])
                    loss_gradient_new[-1][-1].append(loss_gradient[i][32+j][k][0])
        loss_gradient_new = np.asarray(loss_gradient_new)
        loss_gradient_new = loss_gradient_new.reshape((loss_gradient_new.shape[0],loss_gradient_new.shape[1],loss_gradient_new.shape[2],1))
        # print(loss_gradient_new.shape,loss_gradient.shape)
        # print(loss_gradient_new[0][-1])
        # print(loss_gradient[0][-1])
        # exit()
        return loss_gradient_new
